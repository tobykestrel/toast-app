...
export default function Location3Screen() {
  ...
  // Load students whose current location is loc003 from AsyncStorage.
  ...
  useEffect(() => {
    async function loadStudents() {
      try {
        ...
        setStudents(stuArray.filter(stu => stu.currentLocID === "loc003"));
      } catch (err) {
        console.error("Failed to load loc003 students:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) { return <Text>Loading students in loc003...</Text>; }
